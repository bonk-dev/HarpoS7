...
public static class AesConsts
{
    public static ReadOnlyMemory<byte> LutSeed { get; }
    public static ReadOnlyMemory<ushort> HarpoHashSeed { get; }

    static AesConsts()
    {
        LutSeed = new byte[512] {
        ...
        // these bytes were ripped from a DLL and I just didn't bother to convert them
        // into a ushort array, which is the way that they are used
        Span<byte> hashSeed = stackalloc byte[512] 
        {
            0x00, 0x00, 0x01, 0xC2, 0x03, 0x84, 0x02, 0x46, 0x07, 0x08, 0x06, 0xCA,
            0x04, 0x8C, 0x05, 0x4E, 0x0E, 0x10, 0x0F, 0xD2, 0x0D, 0x94, 0x0C, 0x56,
            0x09, 0x18, 0x08, 0xDA, 0x0A, 0x9C, 0x0B, 0x5E, 0x1C, 0x20, 0x1D, 0xE2,
        ...
        var seedBytes = MemoryMarshal.Cast<byte, ushort>(hashSeed);
        var seedMemory = new Memory<ushort>(new ushort[seedBytes.Length]);
        seedBytes.CopyTo(seedMemory.Span);

        HarpoHashSeed = seedMemory;
    }
}